CREATE TABLE IF NOT EXISTS support_threads (
  ...
  category text NOT NULL DEFAULT 'LEARNING_HELP',
  subject text,
  title text NOT NULL,
  status text NOT NULL DEFAULT 'OPEN' CHECK (status IN ('OPEN','WAITING_ON_STUDENT','WAITING_ON_SUPPORT','RESOLVED','CLOSED')),
  priority text NOT NULL DEFAULT 'NORMAL' CHECK (priority IN ('LOW','NORMAL','HIGH','URGENT')),
  assigned_admin_id uuid REFERENCES users(id) ON DELETE SET NULL,
  class_level_snapshot text,
  current_topic_snapshot text,
